        ...
        value: function (require, exports, module) {
            ...
            kity.extendClass(Minder, {
                ...
                importData: function (protocolName, data, option) {
                    ...
                    if (option == 0) //如果是xmind, mindmanager二进制文件protocol已实现异步处理，无需再promise包裹
        ...
        value: function (require, exports, module) {
            ...
            var resolver = function (cb, next, method) {
                return function (value) {
                    if (typeof cb !== "function") next[method].call(next, value); else {
                        var result;
                        try {
                ...
                if (typeof then === "function") {
                    ...
                    try {
                        then.call(x, function (y) {
                            if (resolved) return;
                            resolved = true;
                            if (y === x) promise.reject(new TypeError("circular thenable chain")); else resolve(promise, y);
                        }, function (r) {
                            if (resolved) return;
                            resolved = true;
                            promise.reject(r);
    ...
    _p[32] = {
        value: function (require, exports, module) {
            ...
            kity.extendClass(Minder, {
                ...
                getTheme: function (node) {
                    return this._theme || this.getOption("defaultTheme") || "fresh-purple";
                },
    ...
    _p[65] = {
        value: function (require, exports, module) {
            ...
            data.registerProtocol("json", module.exports = {
                fileDescription: "KityMinder Json",
                fileExtension: ".json",
                dataType: "text",
    ...
    _p[66] = {
        value: function (require, exports, module) {
            ...
            data.registerProtocol("markdown", module.exports = {
                fileDescription: "Markdown/GFM Format",
                fileExtension: ".md",
    ...
    _p[67] = {
        value: function (require, exports, module) {
            ...
            data.registerProtocol("mindmanager", module.exports = {
                fileDescription: "MindManager Format",
                fileExtension: ".mmap",
                dataType: "blob",
            ...
            async function decode(file) {
                ...
                while (entry = entries.pop()) {
                    //查找文档入口content.json
                    if (entry.filename.split("/").pop() == "Document.xml") {
                        entryFile = entry;
                        continue;
                    } else if (entry.filename.split("/").shift() == "bin" && entry.filename.split(".").pop() == "bin") {
                        imageEntries[entry.filename] = entry;
                        continue;
                    }
                }
                //入口文档读取数据
                if (entryFile) {
                    var entryText = await entryFile.getData(new zip.TextWriter());
                    json = $.xml2json(entryText)
            ...
            async function json2km(json, imageEntries) {
                ...
                kmjson.template = "default";
                kmjson.theme = "fresh-purple";
                kmjson.version = Minder.version;
                var topic = json.OneTopic.Topic;
                await processTopic(topic, kmjson.root, imageEntries);
                return kmjson;
            }
            async function processTopic(topic, obj, imageEntries) {
                ...
                if (topic.Hyperlink) {
                    ...
                    obj.data.hyperlinkTitle = topic.Hyperlink.Title;
                }
                //note
                if (topic.NotesGroup)
                    obj.data.note = topic.NotesGroup.NotesXhtmlData.PreviewPlainText;
                //lable
                ...
                //image
                if (topic.OneImage && imageEntries) {
                    imageEntry = imageEntries[topic.OneImage.Image.ImageData.Uri.split("mmarch://").pop()];
                    obj.data.image = await imageEntry.getData(new zip.Data64URIWriter());
                    obj.data.imageSize = {};
                    obj.data.imageSize.width = topic.OneImage.Image.ImageSize.Width;
                    obj.data.imageSize.height = topic.OneImage.Image.ImageSize.Height;
                }
                //children topic
                var topics = topic.SubTopics && topic.SubTopics.Topic;
                if (topics) {
                    var tmp = topics;
    ...
    _p[71] = {
        value: function (require, exports, module) {
            ...
            async function decode(file) {
                ...
                var imageEntries = {};
                while (entry = entries.pop()) {
                    var surfix = entry.filename.split(".").pop();
                    //查找文档入口content.json
                    if (entry.filename.split("/").pop() == "content.json") {
                        entryFile = entry;
                        continue;
                    } else if (entry.filename.split("/").shift() == "resources"
                        && (surfix == "jpeg" || surfix == "jpg" || surfix == "png")) {
                        imageEntries[entry.filename] = entry;
                        continue;
                    }
                }
                ...
                if (entryFile) {
                    var entryText = await entryFile.getData(new zip.TextWriter());
                    return await json2km(JSON.parse(entryText), imageEntries);
                }
                else
                    console.log(new Error("Xmind File corruption!"));
            ...
            async function json2km(json, imageEntries) {
                ...
                kmjson.theme = "fresh-purple";
                kmjson.version = Minder.version;
                var topic = Array.isArray(json) ? json[0].rootTopic : json.rootTopic;
                await processTopic(topic, kmjson.root, imageEntries);
                return kmjson;
            }
            async function processTopic(topic, obj, imageEntries) {
                ...
                //lable
                if (topic.labels) obj.data.resource = topic.labels;
                //image
                if (topic.image && imageEntries) {
                    imageEntry = imageEntries[topic.image.src.split(":").pop()];
                    obj.data.image = await imageEntry.getData(new zip.Data64URIWriter());
            ...
            //get image file size
            function getImgSize(base64Data) {
                return new Promise(function (resolve, reject) {
                    var image = new Image();
