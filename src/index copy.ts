...
function buildDialogHTML(diagramBody: string, language: string): string {
	...
	// return `
	// <script>
	// const iframe = document.getElementById("mindmap_iframe");
	// const iframeDocument = iframe.contentDocument;
	// iframeDocument._lang_default = "${language}";
	// iframeDocument.mindmap_diagram_json = "${diagramBody}";
	// iframeDocument.mindmap_diagram_png = "";
	// alert(iframeDocument._lang_default);
	// alert(iframeDocument.mindmap_diagram_json);
	// </script>
	// `;
}
...
joplin.plugins.register({
	onStart: async function () {
		...
		let htmlPath = (await joplin.plugins.installationDir()) + "/local-kity-minder/index.html";
		...
		async function open_edit_dlg(data_json: string, diagramId: string, type: string = "addnew") {
			...
			//await dialogs.setHtml(handle_dlg, baseHtml);
